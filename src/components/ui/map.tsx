// Adapted for Vite + React (was originally Next.js).
...
//   - removed `next-themes` — pass `theme="light" | "dark"` as a prop instead
...
interface MapProps {
  ...
  loop?: boolean;
  /** "light" | "dark" — controls background and dot colours */
  theme?: 'light' | 'dark';
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Animated curved paths between start/end points ──────────── */}
        {dots.map((dot, i) => {
        ...
        {/* ── Start + end markers (with pulse + hover) ────────────────── */}
        {dots.map((dot, i) => {
          const startPoint = projectPoint(dot.start.lat, dot.start.lng);
          const endPoint   = projectPoint(dot.end.lat,   dot.end.lng);
                    ...
                    <foreignObject x={startPoint.x - 50} y={startPoint.y - 35} width={100} height={30}>
                      <div className="flex items-center justify-center h-full">
                        <span className={`text-sm font-medium px-2 py-0.5 rounded-md ${
                          theme === 'dark'
                            ? 'bg-black/95 text-white border border-gray-700'
